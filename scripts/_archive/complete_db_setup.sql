-- MASTER SETUP SCRIPT
-- Executes all necessary schema changes for "Elite" functionality

-- SECTION 1: ROOMS CONFIGURATION (New Table)
-- Creates the table to store generic room logic (prices, defaults)
create table if not exists rooms (
  id text primary key,
  location text not null check (location in ('pueblo', 'hideout')),
  type text not null check (type in ('dorm', 'private', 'suite')),
  label text not null,
  capacity int not null default 1,     -- Inventory
  max_guests int not null default 1,   -- Physical API Limit
  base_price numeric not null default 0,
  created_at timestamp with time zone default timezone('utc'::text, now()) not null,
  updated_at timestamp with time zone default timezone('utc'::text, now()) not null
);

-- Enable RLS for Rooms
alter table rooms enable row level security;

-- Policies for Rooms
create policy "Public can view rooms" on rooms for select to anon, authenticated using (true);
create policy "Admins can update rooms" on rooms for update to authenticated using (true) with check (true);

-- Insert DEFAULT DATA (Professional/Real Defaults)
insert into rooms (id, location, type, label, capacity, max_guests, base_price) values
  ('pueblo_dorm', 'pueblo', 'dorm', 'Dormitorio Pueblo', 8, 8, 18),
  ('pueblo_private', 'pueblo', 'private', 'Habitación Privada Pueblo', 4, 2, 35),
  ('pueblo_suite', 'pueblo', 'suite', 'Suite Pueblo', 1, 4, 55),
  ('hideout_dorm', 'hideout', 'dorm', 'Dormitorio Hideout', 6, 6, 16),
  ('hideout_private', 'hideout', 'private', 'Habitación Privada Hideout', 3, 2, 40),
  ('hideout_suite', 'hideout', 'suite', 'Suite Hideout', 2, 4, 55)
on conflict (id) do update set
  capacity = excluded.capacity,
  max_guests = excluded.max_guests,
  base_price = excluded.base_price;


-- SECTION 2: BOOKINGS UPDATES (Check-Out Features)
-- Updates existing bookings table to support new check-out/debt UI

-- Add new columns if they don't exist
alter table public.bookings 
add column if not exists actual_check_out timestamp with time zone,
add column if not exists payment_status text default 'pending';

-- Update Constraints for Status (to allow 'checked_out')
alter table public.bookings drop constraint if exists bookings_status_check;
alter table public.bookings add constraint bookings_status_check 
  check (status in ('pending', 'confirmed', 'cancelled', 'checked_out'));
